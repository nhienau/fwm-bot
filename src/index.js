...
const messageHandlersPath = path.join(__dirname, "messageHandlers");
const messageHandlersFiles = fs
  .readdirSync(messageHandlersPath)
  .filter((file) => file.endsWith(".js"));
for (const file of messageHandlersFiles) {
  const filePath = path.join(messageHandlersPath, file);
